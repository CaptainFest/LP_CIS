...
if __name__ == '__main__':
    ...
    loop_ocr_folders.pop('Ru', 'def')
    loop_ocr_folders.pop('Su', 'def')
    ...
    for key in loop_ocr_folders:
        ...
        try:
            train_df = pd.read_csv(os.path.join('data/AmEuGeKgKzMdUa_latinOCR', 'train_df.csv'))
        ...
        train_dataset = IAMDataset(root_dir=os.path.join('data/AmEuGeKgKzMdUa_latinOCR/train', 'img/'),
                                   df=train_df,
                                   processor=processor,
                                   train=True,
                                   aug=args.aug,
        ...
        output_dir = f'exps/{args.exp_name}_{key}' if args.exp_name != 'from_args' else \
                     f'exps/{args.tune}_trocr_base_{key}_{args.train_batch_size}_{args.max_steps}_{args.eval_steps}'
